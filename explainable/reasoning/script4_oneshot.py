...
def parse_sections(raw: str) -> dict:
    """
    Extract [FACTOR1-3], [CONCLUSION] from raw LLM output.
    Supports both closed tags [/TAG] and open-only style (match until next tag).
    Falls back to empty string per section if delimiter is missing.
    """
    keys = ["FACTOR1", "FACTOR2", "FACTOR3", "CONCLUSION"]
    sections = {}

    for i, key in enumerate(keys):
        # Try closed tag first e.g. [FACTOR1]...[/FACTOR1]
        closed = re.search(rf"\[{key}\](.*?)\[/{key}\]", raw, re.DOTALL)
        if closed:
            sections[key.lower()] = clean_text(closed.group(1))
            continue

        # Fallback: match from [KEY] until the next [NEXTKEY] or end of string
        if i + 1 < len(keys):
            next_key = keys[i + 1]
