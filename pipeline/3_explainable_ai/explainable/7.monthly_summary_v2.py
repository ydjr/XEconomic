...
def dedup_existing() -> None:
    """Remove duplicate (Month, Aspect) rows from existing CSV, keeping the last entry."""
    if not OUTPUT_CSV.exists():
        return
    try:
        df = pd.read_csv(OUTPUT_CSV, encoding="utf-8-sig")
        before = len(df)
        df = df.drop_duplicates(subset=["Month", "Aspect"], keep="last").reset_index(drop=True)
        after = len(df)
        if before != after:
            df.to_csv(OUTPUT_CSV, index=False, encoding="utf-8-sig")
            print(f"  [dedup] monthly_summary: removed {before - after} duplicate rows ({after} remaining)")
    except Exception as e:
        print(f"  [WARN] dedup_existing failed: {e}")

def load_done_keys() -> dict:
    """Load completed (Month, Aspect) -> Total_News counts.

    Returning the stored article count (not just the key) enables backfill:
    when an older month gains new articles later, the stored count no
    longer matches the current one, so that month+aspect is re-summarized
    ...
    if OUTPUT_CSV.exists():
        try:
            df = pd.read_csv(OUTPUT_CSV, encoding="utf-8-sig")
            if {"Month", "Aspect", "Total_News"}.issubset(df.columns):
                for _, row in df.iterrows():
                    key = (str(row["Month"]), str(row["Aspect"]))
                    try:
                        done[key] = int(row["Total_News"])
                    except (ValueError, TypeError):
                        done[key] = -1   # unknown count -> force reprocess
        except Exception:
            pass
    return done
...
def main():
    ...
    for month in tqdm(months, desc="Months"):
        ...
        for aspect in tqdm(ASPECTS, desc=f"  {month}", leave=False):
            ...
            # Count-aware resume / backfill: skip only when this month+aspect
            # was already summarized with the SAME article count. A changed
            # count means new (backfilled) articles arrived -> re-summarize.
            if done_keys.get(key) == total_count:
                continue
            ...
            append_row(row)
            done_keys[key] = total_count

    # A re-summarized month+aspect was appended as a fresh row on top of its
    # stale one — collapse those duplicates, keeping the latest.
    dedup_existing()
