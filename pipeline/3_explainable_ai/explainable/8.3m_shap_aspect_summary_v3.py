...
def main():
    ...
    # Dedup monthly_summary by (Month, Aspect) — keep last written entry
    before = len(df_monthly)
    df_monthly = df_monthly.drop_duplicates(subset=["Month", "Aspect"], keep="last").reset_index(drop=True)
    if len(df_monthly) != before:
        print(f"  [dedup] monthly_summary input: removed {before - len(df_monthly)} duplicate rows")
