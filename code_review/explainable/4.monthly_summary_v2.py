"""
Monthly Aspect Sentiment Summarization 
For each month:
  For each aspect:

    1. Filter news for this month + aspect
    2. Split by impact_type:
         Positive group → pos_summaries list
         Negative group → neg_summaries list
         Neutral        → EXCLUDED entirely (too ambiguous)

    3. Compute Overall_Lean (no LLM):
         pos > neg  → บวก
         neg > pos  → ลบ
         equal / both 0 → ทรงตัว

    4. Summarize positive side independently
    5. Summarize negative side independently

    6. Save row to CSV

    
n = number of news on this side

if n == 0:
    → leave blank, no LLM call

if n == 1:
    → copy that single news directly, no LLM call

if n >= 2:
    count total tokens of all news

    if total tokens <= 6000 (INPUT_TOKEN_BUDGET):
        → single LLM call with all news at once

    if total tokens > 6000:
        → MAP-REDUCE:
            1. chunk news into groups of ~2000 tokens each
            2. LLM summarizes each chunk separately (MAP)
            3. LLM combines all chunk summaries into one final summary (REDUCE)
"""
