"""
Script 3 — 3M Summary (with LAG/Indicator Alignment Check + LLM)
================================================================

BEHAVIOR:
  For each target month:
    1. Look up top K SHAP features → indicators that matter most for that month
    2. Map each feature → aspect(s) via FEATURE_ASPECT_MAP
       e.g. cpi → [เศรษฐกิจไทย, ราคาสินค้าเกษตร, ราคาน้ำมันเชื้อเพลิง]
    3. Pull 3 months of monthly summary for those mapped aspects
    4. Call LLM to summarize the pulled content into pos/neg signals
       - LLM is told the indicator definition so it understands context
       - LLM MUST first check whether the monthly evidence aligns with the indicator definition
       - If NOT aligned, LLM must return a fixed fallback sentence instead of forcing a summary
    5. Save one row per feature per month

INPUTS:
  1. monthly_summary CSV (script 2 output)
     - Month, Aspect, Overall_Lean, Positive_Summary, Negative_Summary
     - Pre-summarized news per aspect per month

  2. SHAP CSV (shap_top3_unique.csv)
     - feature, shap_value, abs_shap, forecast_month
     - Top 3 unique SHAP features per forecast month
     - Tells us which indicators matter most

  3. pred_direction CSV
     - date, predicted, direction
     - Forecasted CCI value and direction per month

MAPPINGS:
  FEATURE_ASPECT_MAP  : indicator → which aspects to pull monthly summary from
  INDICATOR_DEFINITIONS: injected into LLM prompt — broad definitions covering all mapped aspects
    e.g. cpi definition covers agriculture prices + energy + general economy
         because all 3 aspects map back to CPI through cost-of-living mechanism

OUTPUT columns:
  Target_Month, Predicted_CCI, CCI_Direction, SHAP_Feature, SHAP_Value,
  ABS_SHAP, SHAP_Direction, Evidence_Aspects, 3M_Trend,
  Summary_3M_Positive, Summary_3M_Negative
"""
